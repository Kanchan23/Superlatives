...
exports.category_create_category = (req, res, next) => {
   ...
   if(role=="admin"){
        ...
        const category = new CropCategory(body);
        category.save()
        .then(result => {
            res.status(201).json({
...
exports.category_update_category = (req, res, next) => {
  CropCategory.findByIdAndUpdate({ _id: req.body._id }, { name:  req.body.name })
     .exec()
     .then(result => {
       res.status(200).json({
...
exports.category_delete_category = (req, res, next) => {
    CropCategory.remove({ _id: req.params.categoryId })
    .exec()
    .then(result => {
      res.status(200).json({
